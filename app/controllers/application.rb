...
class ApplicationController < ActionController::Base
  ...
  session :session_key => '_gastosmini_mor_session_id'
  ...
  helper_method :logged_in_as_guest? #included from authenticated system
  ...
  # request public pages to facturagem.com directly. We need to respond
  ...
  def devalert(subject, body='', extra_to=nil)
    Mailer.deliver_devalert("[GASTOSmini] #{subject}", body, extra_to) if RAILS_ENV == 'production'
  end

  def calculate_total_expenses
    @total = total("expenses.account_id = #{@current_account.id}")
  end
  private :calculate_total_expenses

  def total(conditions)
    Expense.sum('amount', :conditions => conditions)
  end
  private :total
