module FormatterHelper
  ...
  def format_number_according_to_target(number_as_string, target)
    ...
    when :latex
      # LaTeX already takes care of the proper minus sign in math mode.
      "$#{number_as_string}$"
    when :text
  ...
  def format_integer(n, options={})
    ...
    n = 0 if n.nil? && options[:force_zero]
    return '' unless n
    n = GastosminiUtils.parse_integer(n) if n.is_a?(String)
  ...
  def format_integer_or_decimal(n, options={})
    GastosminiUtils.integer?(n) ? format_integer(n, options) : format_decimal(n, options).sub(/0+$/, '')
  end
