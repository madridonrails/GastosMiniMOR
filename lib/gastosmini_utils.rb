...
module GastosminiUtils
  ...
  # This method understands dd-mm-yyyy. Returns parse_date on failure
  def self.parse_csv_date(str)
    day, month, year = str.split("-")
    begin
      return Date.new(year.to_i, month.to_i, day.to_i)
    rescue
      return self.parse_date(str)
    end
  end
  ...
  def self.integer?(n)
    n = GastosminiUtils.parse_decimal(n) if n.is_a?(String)
    return n == n.to_i
  end
  ...
  def self.nif?(code)
    return false unless code =~ /\A([0-9]+)([A-Z])\z/
  ...
  def self.bank_account?(account_number)
    ...
    bank_office, control, account = account_number.unpack('a8a2a10')
...
class FckeditorListener
  ...
  def text(text)
    self.latex << GastosminiUtils.escape_latex(text)
  end
